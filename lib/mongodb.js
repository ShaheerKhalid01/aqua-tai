...
function saveDataToFile(data) {
  try {
    ...
    try {
      const verifyData = JSON.parse(readFileSync(DB_FILE, 'utf8'));
      console.log('✅ File verification - Orders count:', verifyData.orders?.length || 0);
    } catch (verifyError) {
...
export async function deleteProduct(id) {
  ...
  console.log('Product at index:', index !== -1 ? products[index] : 'Not found');
  
  if (index !== -1) {
    const deletedProduct = products.splice(index, 1)[0];
    console.log('Product spliced from array:', deletedProduct);
    console.log('Products count after splice:', products.length);
    
    saveAfterChange(); // Save to file
    console.log('saveAfterChange called');
    
    // Verify the save worked
    setTimeout(() => {
      console.log('Verifying save - current products count:', products.length);
    }, 100);
...
export async function deleteOrder(id) {
  console.log('=== DELETE ORDER FUNCTION CALLED ===');
  ...
  console.log('Order at index:', index !== -1 ? orders[index] : 'Not found');
  
  if (index !== -1) {
    const deletedOrder = orders.splice(index, 1)[0];
    console.log('Order spliced from array:', deletedOrder);
    console.log('Orders count after splice:', orders.length);
    ...
    // Verify the save worked
    setTimeout(() => {
      console.log('Verifying save - current orders count:', orders.length);
    }, 100);

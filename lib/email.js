...
export const verifyResetToken = (email, token) => {
  ...
  try {
    ...
    console.log('Reset links path:', resetLinksPath);
    console.log('File exists:', fs.existsSync(resetLinksPath));
    
    if (fs.existsSync(resetLinksPath)) {
      const links = JSON.parse(fs.readFileSync(resetLinksPath, 'utf8'));
      console.log('Total links in file:', links.length);
      console.log('All links:', links);
      
      const storedToken = links.find(link => link.email === email && link.token === token);
      console.log('Found matching link:', storedToken);
      
      if (!storedToken) {
        console.log('❌ No matching email and token found');
        return false;
      }
      
      const now = new Date();
      const expiry = new Date(storedToken.expiry);
  ...
  console.log('Trying global memory fallback...');
  const resetTokens = global.resetTokens || {};
  const storedToken = resetTokens[email];
  
  console.log('Global reset tokens:', resetTokens);
  console.log('Stored token for email:', storedToken);
  
  if (!storedToken || storedToken.token !== token) {
    console.log('❌ No matching token in global memory');
    return false;
  }
  
  if (new Date() > storedToken.expiry) {
    console.log('❌ Token expired in global memory');
    delete resetTokens[email];
    return false;
  }
  
  console.log('✅ Token valid in global memory');
  return true;
};

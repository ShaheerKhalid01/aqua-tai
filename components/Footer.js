...
function FooterSection({ title, children }) {
  ...
  return (
    <div className="footer-section">
      {/* Collapsible toggle (mobile only, controlled by CSS) */}
      <button className="footer-section-toggle" onClick={() => setOpen(!open)}>
        {title}
        <span className={`toggle-icon ${open ? "open" : ""}`}>▼</span>
      </button>
      {/* Title for desktop (hidden on mobile by CSS) */}
      <h4 style={{ color: "#fff", fontWeight: 800, fontSize: 14, marginBottom: 18, letterSpacing: 1 }}>{title}</h4>
          ...
          <div className="footer-brand">
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
              ...
              <div>
                ...
                <div style={{ color: "#94a3b8", fontSize: 10, letterSpacing: 2 }}>PURE WATER - HEALTHY LIFE</div>
              </div>
            </div>
